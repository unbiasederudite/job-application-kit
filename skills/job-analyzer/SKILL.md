...
Turns one job posting into a structured brief that `tailor-cv` and `cover-letter` can work from, and creates the folder that holds everything for that application.
...
Create:
```
data/applications/<slug>/
  posting.md      # the raw posting, verbatim (title, company, and full description text)
  analysis.md      # your structured extraction, see below
```
...
Extract, don't editorialize — pull language straight from the posting for must-haves and keywords so `tailor-cv` can mirror real ATS terms instead of paraphrases. Save the editorializing (fit assessment, gap flags) for the sections that call for it.

## After analysis

Tell the user what you found — a short summary of must-haves and any notable gaps — and ask if they want to proceed straight to `tailor-cv` and `cover-letter`, or review/correct `analysis.md` first. Don't chain into tailoring automatically; that's what the `/apply` command and the orchestrator agent are for when the user wants the full pipeline in one go.
